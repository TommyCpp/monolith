...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("without order", |b| {
        b.iter(|| intersect_vec(vec![1, 2, 3], vec![1, 3, 4, 5, 6]))
    });
    c.bench_function("with order", |b| {
        b.iter(|| intersect_order_vec(vec![1, 2, 3], vec![1, 3, 4, 5, 6]))
    });
}
