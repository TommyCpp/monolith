...
impl Storage for TiKvStorage {
    fn write_time_point(&self, time_series_id: TimeSeriesId, timestamp: Timestamp, value: Value) -> Result<()> {
        ...
        if let Some(mut val) = self.client.get(key.clone())? {
            ...
            let last_time_point = KvStorageProcessor::decode_time_point(&val[(val.len() - timepoint_size)..])?;
...
mod tests {
    ...
    fn test_write_timestamp() -> Result<()> {
        ...
        assert!(val.is_some());
        assert_eq!(val.unwrap(), KvStorageProcessor::encode_time_point(123u64, 56.7)?);
    ...
    fn test_read_time_series() -> Result<()>{
        ...
        for (ts, v) in vec![(120u64, 12.0), (123, 16.7), (156, 89.0), (190, 10.0)]{
            val.append(&mut Vec::from(KvStorageProcessor::encode_time_point(ts, v)?));
        }
        dummy_backend.set(key.clone(), val.clone());

        let res = storage.read_time_series(1, 120, 160)?;
        let res_flat = res.iter()
